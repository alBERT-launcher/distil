...
router.get(
  "/pipelines/:name/versions/:id/generations",
...
router.get(
  "/pipelines/:name/versions/:id/generations/:genId",
  async (req: Request, res: Response) => {
    try {
...
router.post(
  "/pipelines/:name/versions/:id/tag",
  async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const { tag } = req.body;
...
router.post(
  "/pipelines/:name/versions/:id/generations/:genId/rate",
  async (req: Request, res: Response) => {
    const { name, id, genId } = req.params;
    const { rating } = req.body;

    const success = await rateGeneration(name, genId, parseInt(rating));
    if (!success) {
...
router.post(
  "/pipelines/:name/versions/:id/generations/finetune",
  async (req: Request, res: Response) => {
    try {
      ...
      if (!Array.isArray(generationIds)) {
        return res
          .status(400)
          .json({ error: "Invalid generationIds parameter" });
      }
      ...
      if (!success) {
        return res
          .status(500)
          .json({ error: "Failed to mark generations for finetuning" });
      }
      ...
      res
        .status(500)
        .json({ error: "Failed to mark generation for finetuning" });
    }
  }
);
